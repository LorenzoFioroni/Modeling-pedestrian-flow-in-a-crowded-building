...
for i in range(50):
    if np.random.rand()<0.5:
        target0 = np.array([0, 4.5])
        target1 = np.array([0, 5.5])
        start = np.array([10, np.random.uniform(4.5, 5.5)])
    else:
        target0 = np.array([10, 4.5])
        target1 = np.array([10, 5.5])
        start = np.array([0, np.random.uniform(4.5, 5.5)])
    sim.add_agent(Agent(env, np.abs(np.random.normal(loc=3, scale=1.5)), start, target0, target1))
